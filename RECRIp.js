(function () {
  // 1️⃣ Jalankan handleGoto() dulu supaya ?goto= selalu aktif
  (function handleGoto() {
  ...
  // 2️⃣ Baru cek allowedPosts untuk proses penggantian link
  ...
  function allowedPostsList() {
    return (setting.allowedPosts || "").split(",").map(s => normalizePath(s)).filter(Boolean);
  }

  if (typeof setting === "undefined") {
    console.warn("setting belum didefinisikan.");
    return;
  }

  const currentPath = normalizePath(window.location.pathname);
  const allowed = allowedPostsList();

  if (allowed.length > 0 && !allowed.includes(currentPath)) {
    console.log("Proses penggantian link dinonaktifkan di halaman ini:", currentPath);
    return;
  }
