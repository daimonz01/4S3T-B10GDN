(function () {
  ...
  function allowedPostsList() {
    var raw = (setting && setting.allowedPosts) ? setting.allowedPosts : "";
    return raw.split(",").map(s => normalizePath(s)).filter(Boolean);
  }
  if (typeof setting === "undefined") {
    console.warn("setting belum didefinisikan. Pastikan <script> setting {...} </script> ditempatkan sebelum script ini.");
    return;
  }
  var currentPath = normalizePath(window.location.pathname);
  var allowed = allowedPostsList();
  if (allowed.length > 0 && !allowed.includes(currentPath)) {
    console.log("Script dinonaktifkan di halaman ini (tidak ada di allowedPosts):", currentPath);
    return;
  }
  (function handleGoto() {
  ...
  window.showurl = function (datajson) {
    var check = false;
    var no = 0;
    ...
    links = links.sort(function(){ return Math.random() - 0.5; });
